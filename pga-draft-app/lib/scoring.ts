...
export function buildPlayerScores(
  ...
  // Use ESPN's parsed cutLine directly.
  // After the cut is made, ESPN re-ranks survivors 1..N where N ≤ cutLine,
  // so cutLine + 1 is always worse than any surviving player's position.
  // The previous survivorCount approach inflated the cut score during the
  // R2→R3 data transition when some cut players still showed as 'active'.
  const effectiveCutLine = cutLine;
